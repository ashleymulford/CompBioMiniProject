...
directory<-paste(directory, "/kallisto_results", sep="")

sample_id <- dir(file.path(directory)) #list of srrs
kal_dirs <- file.path(directory, sample_id) #list of paths
...
sleuth_significant <- dplyr::filter(sleuth_table, pval <= 0.05) #filter for signifcance
save_sleuth_significant<-sleuth_significant[1:4] #extract first four columns

#output to log
write.table(save_sleuth_significant, "project.log", append = TRUE, sep = "\t", col.names = TRUE, quote = FALSE, row.names = FALSE)
